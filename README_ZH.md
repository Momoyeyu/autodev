...
| 阶段 | 功能开发 | 性能优化 |
|---|---|---|
| **Clarify · 对齐** | 确认影响；共同准备可执行 test 直到用户确认；记录 baseline | 确认唯一的可执行数值 test；记录 baseline；确认目标、可编辑文件和时间预算 |
| **Loop · 迭代** | 开发直到确认后的 test 全部通过 | 优化直到达标或时间耗尽 |
| **Handoff · 交付** | 一张表，对比 baseline 与最终 test 结果 | 一张图，展示 baseline、优化尝试和最终结果 |

Clarify 不只是问问题。查代码、维护测试、试运行和测量 baseline，都是建立共识的一部分。Loop 负责实现共识，而不是边写代码边重新解释成功标准。
...
“能执行”不等于“已经全部通过”：尚未实现的功能可以在准备和 baseline 阶段失败；损坏的运行环境不是有效证据。正式 baseline 应在用户确认之后、功能实现之前记录，不能把删除过时测试算作开发收益。
...
**一项 test、一个分数：** 确认可执行的 benchmark 或固定加权和。沿实线从测试准备走到 baseline 与限制确认，沿虚线反复审阅或优化。Loop 采用 **do-while** 顺序：先优化、测量并保留已验证的最佳结果，再判断是否达标或时间耗尽；均未满足才继续下一轮。每次运行仍受剩余时间预算约束。
