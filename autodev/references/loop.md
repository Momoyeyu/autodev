...
4. Keep the attempt only if it is accepted under the direction-bound rule from Clarify (lower: `score < best - δ`; higher: `score > best + δ`); otherwise restore only that attempt's changes to the best valid state.
